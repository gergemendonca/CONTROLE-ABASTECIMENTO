...
export async function GET(req:Request){
 ...
 try{
  const rows=await env.DB!.prepare(`SELECT f.id,v.label AS vehicleLabel,t.route,f.driver,f.created_at AS createdAt,f.amount_cents AS amountCents,f.liters,COALESCE(a.name,'Não informado') AS authorizedBy,fr.authorized_at AS authorizedAt FROM fueling f JOIN vehicles v ON v.id=f.vehicle_id JOIN trips t ON t.id=f.trip_id LEFT JOIN fuel_requests fr ON fr.id=f.fuel_request_id LEFT JOIN app_users a ON a.id=fr.authorized_by ORDER BY f.created_at DESC,f.id DESC`).all();
